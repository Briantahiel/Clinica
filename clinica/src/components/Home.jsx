...
//       const citasResponse = await fetch(`http://localhost:5000/api/citas?paciente_id=${pacienteID}`);
...
//                 {citas.map((cita) => (
//                   <li key={cita.fecha}>
//                     Cita con {cita.medico_id} el {cita.fecha}
//                   </li>
//                 ))}
//               </ul>
//             ) : (
...
const Home = () => {
  ...
  const fetchCitasPaciente = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/citas?paciente_id=${pacienteID}&fecha=${selectedFecha}&medico_id=${selectedMedico.id}`
      );
      ...
      console.error(error);
      alert("Hubo un error al obtener las citas del paciente");
    }
  };
  ...
  return (
    <div>
      ...
      <button onClick={fetchCitasPaciente}>Ver Citas</button>
      <div>
  <h3>Citas del Paciente:</h3>
  {citasPaciente.length > 0 ? (
    <ul>
      {citasPaciente.map((cita) => (
        <li key={cita.id}>{cita.id}</li>
      ))}
    </ul>
  ) : (
